...
def makePath(player):
    ...
    while s.size() > 0:
        ...
        if current not in explored:
            ...
            # randomWalk(exits, graph, s, player, current)

            if 'n' in graph[current]:
                print ('Going North')
                player.travel('n')
                graph[current]['n'] = player.currentRoom.id
                s.push(player.currentRoom.id)
            elif 'e' in graph[current]:
                print ('Going East')
                player.travel('e')
                graph[current]['e'] = player.currentRoom.id
                s.push(player.currentRoom.id)
            elif 's' in graph[current]:
                print ('Going South')
                player.travel('s')
                graph[current]['s'] = player.currentRoom.id
                s.push(player.currentRoom.id)
            elif 'w' in graph[current]:
                print ('Going West')
                player.travel('w')
...
def randomWalk(arr, graph, stack, player, current):
    for i in arr:
        if graph[current]['i'] == '?':
            player.travel('n')
            graph[current]['n'] = player.currentRoom.id
